...
///</summary>
SceneMgr::SceneMgr(Keyboard* keyboard,Sound* sound) : next_scene_(Scene::Scene_None) //次のシーン管理変数
{
	keyboard_ = keyboard;
	sound_ = sound;
	scene_ = (BaseScene*) new Title(keyboard_, sound_, this);
	scene_->StartBgm();
	state = SceneState::Normal_Scene;
	screen = { 0,0,640,480 };
}
...
void SceneMgr::Update() {
	switch (state) {
	case SceneState::Normal_Scene:
		if (next_scene_ != Scene::Scene_None) {    //次のシーンがセットされていたら
			state = SceneState::BlackOut_Scene;
			sound_->StopBgm();
			sound_->StopSe();
			sound_->PlaySe(Se_Name::Se_SceneChange);
		}
		scene_->Update(); //シーンの更新
		break;
	case SceneState::BlackOut_Scene:
		BlackOutScene();
		break;
	case SceneState::Lighting_Scene:
		LightingScene();
		break;
	default:
		assert(false);
...
void SceneMgr::BlackOutScene() {
	...
	if (screen.x1 > screen.x2 || screen.y1 > screen.y2) {
		...
		next_scene_ = Scene::Scene_None;    //次のシーン情報をクリア
